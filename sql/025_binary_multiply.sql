...
-- ============================================================================
-- py_float_nb_multiply(left_id, right_id) — float * float, float * int (CPython coercion)
--    left는 py_float_object. right가 float 또는 int(변환 후 곱셈). 그 외 NotImplemented.
-- ============================================================================

CREATE OR REPLACE FUNCTION public.py_float_nb_multiply(left_id uuid, right_id uuid)
RETURNS uuid AS $$
DECLARE
    result_id uuid;
    lv double precision;
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM public.py_unicode_object WHERE ob_base = seq_id) THEN
        PERFORM public.py_err_set_type_error('py_unicode_sq_repeat operand is not str');
        RETURN NULL;
    END IF;
...
BEGIN
    ...
    UPDATE public.py_number_methods
    SET nb_multiply = 'py_float_nb_multiply'::regproc
