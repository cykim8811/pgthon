...
-- ============================================================================
-- F: py_float_nb_add(left_id, right_id) — float + float, float + int (CPython coercion)
--    left는 py_float_object. right가 float면 ob_fval 끼리 덧셈; right가 int면 long_value를 double로 변환 후 덧셈. 그 외 NotImplemented.
-- ============================================================================

CREATE OR REPLACE FUNCTION public.py_float_nb_add(left_id uuid, right_id uuid)
RETURNS uuid AS $$
DECLARE
    result_id uuid;
    lv double precision;
...
BEGIN
    ...
    UPDATE public.py_number_methods
    SET nb_add = 'py_float_nb_add'::regproc
...
-- Opcode BINARY_ADD (23): 20260114238001_opcode_binary_add.sql
