...
-- Supports: list (int index, remove element at index), dict (py_dict_del_item).
-- tuple → TypeError (does not support item deletion).
-- Depends: ceval_core, py_list_object, py_dict_del_item, py_long_object, exception setters.
-- ============================================================================
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM public.py_tuple_object WHERE ob_base = obj_id) THEN
        PERFORM public.py_err_set_type_error('tuple object does not support item deletion');
        RETURN;
    ...
    ELSE
        ...
        PERFORM public.py_err_set_type_error('''' || COALESCE(type_name, 'unknown') || ''' object does not support item deletion');
        RETURN;
    END IF;
END;
$$ LANGUAGE plpgsql;
