-- ============================================================================
-- Exception helpers: set/clear/query state, traceback (CPython 3.11)
-- 20260114240800_exception_helpers.sql
--
-- Design: docs/EXCEPTION_HANDLING_DESIGN.md
-- - py_err_set_object(type_id, value_id): set error indicator
-- - py_err_clear(): clear exception state
-- - py_err_occurred(): true if exception is set
-- - py_err_get_raised(): (exc_type_id, exc_value_id, exc_traceback_id)
-- - py_traceback_here(frame_id, lasti): prepend current frame to traceback
-- ============================================================================

-- Single row id for py_exception_state: 00000000-0000-4000-e000-000000000001
-- TracebackType: 00000000-0000-4000-a000-000000000025 (bootstrap in 20260114240700)
...
BEGIN
  ...
  IF NOT FOUND OR (SELECT exc_type_id FROM public.py_exception_state WHERE id = '00000000-0000-4000-e000-000000000001') IS NULL THEN
    RETURN; /* no exception set, nothing to do */
  END IF;
