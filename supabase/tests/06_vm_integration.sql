...
BEGIN
    ...
    bytecode := CHR(100) || CHR(0) || CHR(100) || CHR(1) || CHR(23) || CHR(0) || CHR(83) || CHR(0);
    
    co_code_id := gen_random_uuid();
    INSERT INTO public.py_object (id, ob_type) VALUES (co_code_id, ID_STR_TYPE);
    INSERT INTO public.py_unicode_object (ob_base, str_value) VALUES (co_code_id, bytecode);
    ...
    i := 1;
    opcode := ascii(substring(bytecode FROM i FOR 1));
    IF opcode != 100 THEN
    ...
    i := 1;
    opcode := ascii(substring(bytecode FROM i FOR 1));
    operand := ascii(substring(bytecode FROM i+1 FOR 1));
    
    IF opcode != 100 OR operand != 0 THEN
        RAISE EXCEPTION 'FAIL: First instruction is not LOAD_CONST 0';
    END IF;
    ...
    IF opcode != 100 OR operand != 1 THEN
        RAISE EXCEPTION 'FAIL: Second instruction is not LOAD_CONST 1';
    END IF;
    ...
    -- Note: We're not actually implementing BINARY_ADD here, just simulating
    -- the stack manipulation part
    i := i + public.py_get_opcode_size(opcode);  -- Move to next instruction
    opcode := ascii(substring(bytecode FROM i FOR 1));
