-- ============================================================================
-- Test: BINARY_ADD Bytecode Integration
--
-- Purpose:
--   Phase 4+5 구현 검증. py_eval_frame으로 바이트코드 실행 시 BINARY_ADD 동작을 확인한다.
--   - 1+2 → int 3
--   - 'a'+'b' → str "ab"
--   - 1+'a' → TypeError (메시지에 'int', 'str' 포함)
--
-- Bytecode: LOAD_CONST(0) LOAD_CONST(1) BINARY_ADD RETURN_VALUE
--   = 100,0, 100,1, 23,0, 83,0  →  \x6400640117005300
--
-- Usage:
--   Run after migrations 238300, 238400. If any assertion fails, exception is raised.
-- ============================================================================
...
BEGIN
    RAISE NOTICE '========================================';
    RAISE NOTICE 'BINARY_ADD Bytecode Integration Test';
    RAISE NOTICE '========================================';
    RAISE NOTICE '';

    -- -------------------------------------------------------------------------
    -- Setup: frame, code, co_consts/co_names/co_code, namespaces
    -- -------------------------------------------------------------------------
    ...
    -- -------------------------------------------------------------------------
    -- Test 1: 1+2 → 3 (LOAD_CONST 0,1 + BINARY_ADD + RETURN_VALUE)
    -- -------------------------------------------------------------------------
    RAISE NOTICE 'Test 1: bytecode 1+2 → 3...';
    ...
    co_code_id := gen_random_uuid();
    INSERT INTO public.py_object (id, ob_type) VALUES (co_code_id, ID_BYTES_TYPE);
    INSERT INTO public.py_bytes_object (ob_base, bytes_value) VALUES (co_code_id, E'\\x6400640117005300'::bytea);
    ...
    IF result_num IS NULL OR result_num <> 3 THEN
        RAISE EXCEPTION 'FAIL: 1+2 bytecode result %, expected 3', result_num;
    END IF;
    RAISE NOTICE '  ✓ 1+2 = 3';
    ...
    -- -------------------------------------------------------------------------
    RAISE NOTICE '';
    RAISE NOTICE 'Test 2: bytecode ''a''+''b'' → "ab"...';
    test_count := test_count + 1;
    ...
    IF result_id IS NULL THEN
        RAISE EXCEPTION 'FAIL: ''a''+''b'' bytecode returned NULL';
    END IF;
    ...
    IF result_txt IS NULL OR result_txt <> 'ab' THEN
        RAISE EXCEPTION 'FAIL: ''a''+''b'' bytecode result %, expected "ab"', COALESCE(result_txt, 'NULL');
    END IF;
    RAISE NOTICE '  ✓ ''a''+''b'' = "ab"';
    pass_count := pass_count + 1;

    -- -------------------------------------------------------------------------
    -- Test 3: 1+''a'' → TypeError (메시지에 int, str 포함)
    -- -------------------------------------------------------------------------
    RAISE NOTICE '';
    RAISE NOTICE 'Test 3: bytecode 1+''a'' raises TypeError...';
    test_count := test_count + 1;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ...
            IF SQLERRM NOT LIKE '%int%' OR SQLERRM NOT LIKE '%str%' THEN
                RAISE EXCEPTION 'FAIL: 1+''a'' exception message should mention int and str: %', SQLERRM;
            END IF;
    END;
    ...
    RAISE NOTICE '✓ All BINARY_ADD bytecode integration tests passed!';
END $$;
