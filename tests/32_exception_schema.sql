...
--   - py_thread_state row and tables (py_base_exception_object, py_traceback_object)
...
DECLARE
  ...
  EXCEPTION_STATE_ID uuid := '00000000-0000-4000-e000-000000000030'; -- thread state
  n integer;
  t uuid;
  v uuid;
  tb uuid;
BEGIN
  -- 1. py_thread_state: single row exists, initially clear
  SELECT count(*) INTO n FROM public.py_thread_state WHERE id = EXCEPTION_STATE_ID;
  IF n <> 1 THEN
    RAISE EXCEPTION '✓ 32.1 FAIL: py_thread_state should have exactly one row';
  END IF;
  IF (SELECT exc_type_id FROM public.py_thread_state WHERE id = EXCEPTION_STATE_ID) IS NOT NULL THEN
    RAISE EXCEPTION '✓ 32.1 FAIL: initial exc_type_id should be NULL';
  END IF;
  RAISE NOTICE '✓ 32.1 py_thread_state row exists and is clear';
  ...
  IF (SELECT exc_traceback_id FROM public.py_thread_state WHERE id = EXCEPTION_STATE_ID) IS NOT NULL THEN
  ...
  IF (SELECT exc_traceback_id FROM public.py_thread_state WHERE id = EXCEPTION_STATE_ID) IS NULL THEN
